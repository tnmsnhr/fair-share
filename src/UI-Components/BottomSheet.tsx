...
export default function BottomSheet({
    ...
    const snaps = useMemo(() => {
        const uniq = Array.from(new Set(snapPoints.map((n) => clamp(n, 0, 1)))).sort((a, b) => a - b);
    ...
    const snapPx = useMemo(() => snaps.map((f) => f * SHEET_HEIGHT), [snaps, SHEET_HEIGHT]);
    ...
    const responder = useRef(
        PanResponder.create({
            onMoveShouldSetPanResponder: (_e, g) => Math.abs(g.dy) > Math.abs(g.dx) && Math.abs(g.dy) > 3,
            onPanResponderGrant: () => {
            ...
            onPanResponderRelease: (_e, g) => {
                ...
                const projected = clamp(currentY.current + g.vy * dragToss, 0, SHEET_HEIGHT);
